...
When a pattern includes YAML frontmatter at the top of `system.md`, Fabric treats that block as pattern metadata rather than prompt text. Obsidian note saves can use that metadata to populate note properties before writing the Markdown body.
...
Patterns can now include an optional leading YAML frontmatter block in `system.md`. That metadata is not sent to the model as part of the prompt body. Instead, Fabric exposes it through the patterns API and uses it for note-saving flows such as Obsidian.
...
If this pattern is used in an Obsidian save flow, the resolved metadata becomes the saved note's frontmatter and the model output is written as the raw Markdown body.
