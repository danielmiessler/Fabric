...
func (c *Client) Setup() error {
	...
	fmt.Println()
	fmt.Println("Starting browser-based OpenAI login for Codex.")
...
func (c *Client) configure() error {
	...
	if strings.TrimSpace(c.RefreshToken.Value) == "" {
		return errors.New("Codex refresh token is required. Run 'fabric --setup' and configure Codex again.")
	}
...
func (c *Client) Send(ctx context.Context, msgs []*chat.ChatCompletionMessage, opts *domain.ChatOptions) (string, error) {
	if opts.ImageFile != "" {
		return "", errors.New("Codex does not support Fabric image generation output. Use the OpenAI vendor for --image-file.")
	}
	if c.ApiClient == nil {
		if err := c.configure(); err != nil {
			return "", err
...
func (c *Client) runOAuthFlow(
	...
	case err := <-serveDone:
		...
		return oauthTokens{}, errors.New("Codex login server stopped before authentication completed")
	case <-ctx.Done():
		shutdownCtx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
		defer cancel()
		_ = server.Shutdown(shutdownCtx)
		<-serveDone
		return oauthTokens{}, errors.New("Timed out waiting for Codex login to complete in the browser")
	}
}
...
func (c *Client) handleOAuthCallback(
	...
	if r.URL.Query().Get("state") != expectedState {
		...
		c.publishOAuthResult(results, oauthResult{
			err: errors.New("Codex login failed because the OAuth callback state did not match"),
		})
		return
	}
	...
	if callbackError := strings.TrimSpace(r.URL.Query().Get("error")); callbackError != "" {
		...
		if description != "" {
			...
			c.publishOAuthResult(results, oauthResult{
				err: fmt.Errorf("Codex login failed: %s", description),
			})
			return
		}
		http.Error(w, callbackError, http.StatusForbidden)
		c.publishOAuthResult(results, oauthResult{
			err: fmt.Errorf("Codex login failed: %s", callbackError),
		})
		return
	}
	...
	if code == "" {
		...
		c.publishOAuthResult(results, oauthResult{
			err: errors.New("Codex login failed because OpenAI did not return an authorization code"),
		})
		return
	}
...
func (c *Client) exchangeCodeForTokens(
	...
	if strings.TrimSpace(tokens.AccessToken) == "" || strings.TrimSpace(tokens.RefreshToken) == "" {
		return oauthTokens{}, errors.New("Codex login completed but OpenAI did not return usable access and refresh tokens")
	}
...
func (c *Client) ensureAccessToken(ctx context.Context, forceRefresh bool) (string, string, error) {
	...
	if accountID != "" && refreshedAccountID != "" && !strings.EqualFold(accountID, refreshedAccountID) {
		return "", "", errors.New("Your Codex login now belongs to a different workspace/account. Run 'fabric --setup' and configure Codex again.")
	}
...
func (c *Client) refreshAccessToken(ctx context.Context) (oauthTokens, error) {
	...
	if strings.TrimSpace(refreshed.AccessToken) == "" {
		return oauthTokens{}, errors.New("Codex token refresh succeeded but did not return an access token")
	}
...
func (c *Client) extractAccountID(idToken string, accessToken string) (string, error) {
	...
	return "", errors.New("Codex login succeeded but the returned token did not include a chatgpt_account_id claim")
}
...
func (c *Client) errorFromHTTPResponse(statusCode int, body []byte) error {
	...
	if statusCode == http.StatusUnauthorized {
		return errors.New("Your Codex login is no longer valid. Run 'fabric --setup' and configure Codex again.")
	}
...
func (c *Client) refreshErrorFromResponse(statusCode int, body []byte) error {
	...
	if statusCode == http.StatusUnauthorized {
		...
		case "refresh_token_expired", "refresh_token_reused", "refresh_token_invalidated":
			return errors.New("Your Codex login has expired or was revoked. Run 'fabric --setup' and configure Codex again.")
		default:
			return errors.New("Your Codex login could not be refreshed. Run 'fabric --setup' and configure Codex again.")
		}
	}
